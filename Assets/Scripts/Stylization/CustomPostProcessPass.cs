...

{
    ...
    private RTHandle m_CamerDepthTarget;
    ...
    public void SetTarget(RTHandle cameraColorTargetHandle, RTHandle cameraDepthTargetHandle)
    {
        ...
        m_CamerDepthTarget = cameraDepthTargetHandle;
    ...
    private void SetUpBloom(CommandBuffer cmd, RTHandle source)
    {
        ...
        int mipCount = Mathf.Clamp(iterations, 1, m_BloomEffect.maxInterations.value);
        ...
        {
            RenderingUtils.ReAllocateIfNeeded(ref m_BloomMipUp[1], desc, FilterMode.Bilinear, TextureWrapMode.Clamp, name: m_BloomMipUp[i].name);
            RenderingUtils.ReAllocateIfNeeded(ref m_BloomMipDown[1], desc, FilterMode.Bilinear, TextureWrapMode.Clamp, name: m_BloomMipDown[i].name);
            desc.width = Mathf.Max(1, desc.width >> 1);
            desc.height = Mathf.Max(1, desc.width >> 1);
        }
        ...
        //Downsample - gaussain pyramid
        var lastDown = m_BloomMipDown[0];
        for (int i = 1; i < mipCount; i++)
        {
            // Calassic two pass gaussian blur - use mipUp as a temporary target
            // First pass does 2x downsampling + 9 -tap gaussian
            //  Second pass does 9-tap gaussian usiang a 5-tap filter + bilinear filtering 

            Debug.Log("Test " + i);
            Debug.Log("CMD " + cmd);
            Debug.Log("LastDown " + lastDown);
            Debug.Log("m_BloomMipUp " + m_BloomMipUp[i]);
        ...
        //Upsample (bilinear by defaut, HQ filtering does bicubic instead
        for (int i = mipCount - 2; i >= 0; i--)
        {
            var lowMip = (1 == mipCount - 2) ? m_BloomMipDown[i + 1] : m_BloomMipUp[i + 1];
